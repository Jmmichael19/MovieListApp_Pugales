...
function App() {
  ...
  // ✅ Centralized fetch function (same style as MovieDetails.jsx)
  ...
  // ✅ Search movies (uses centralized fetch)
  const fetchMovies = async (term = searchTerm) => {
    const data = await fetchFromAPI({ s: term || "Avengers" });
    setMovies(data?.Search || []);
  };

  // ✅ Add favorite (fetch full details like MovieDetails page)
  const addFavorite = async (movie) => {
    setAddingFavorites((s) => (s.includes(movie.imdbID) ? s : [...s, movie.imdbID]));
    try {
      const details = await fetchFromAPI({ i: movie.imdbID, plot: "full" });
      const movieData = details || movie;
      setFavorites((prev) => {
        if (prev.some((m) => m.imdbID === movieData.imdbID)) return prev;
      ...
      addToast(`${movie.Title} added to favorites (offline mode)`, "info");
    } finally {
      ...
      <main className="container mx-auto px-4 py-10 w-full max-w-6xl min-h-[60vh] flex flex-col items-center justify-center relative z-10">
        ...
        <div className="flex flex-col items-center w-full gap-6">
          ...
          <div className="mt-2">
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-white group-hover:animate-pulse" viewBox="0 0 20 20" fill="currentColor">
              ...
              {favorites.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {favorites.map((m) => (
                    <div
                      key={m.imdbID}
                      className="bg-slate-800/50 backdrop-blur rounded-xl shadow-lg p-4 border border-slate-700/50 hover:border-purple-500/50 transition-all duration-300 hover:shadow-purple-500/20 hover:shadow-xl transform hover:-translate-y-1"
                    >
